    ...
    <form action="{{ route('admin.routes.update', $route) }}" method="POST" id="routeForm">
        ...
        <div class="card-body">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input 
                            ...
                            id="name" 
                            name="name" 
                            placeholder="e.g., Maa - Agdao - SM Lanang"
                    ...
                    <div class="form-group">
                        ...
                        <input 
                            ...
                            id="terminal" 
                            name="terminal" 
                            placeholder="e.g., Maa Terminal"
                            value="{{ old('terminal', $route->terminal ?? $route->start_point) }}"
                            required
                        >
                        @error('terminal')
                            <span class="error-text">{{ $message }}</span>
                    ...
                    <div class="form-group">
                        ...
                        <select 
                            ...
                            required
                        >
                            <option value="available" {{ old('status', $route->status) === 'available' ? 'selected' : '' }}>Available</option>
                            <option value="unavailable" {{ old('status', $route->status) === 'unavailable' ? 'selected' : '' }}>Unavailable</option>
                        </select>
                        @error('status')
                            <span class="error-text">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <div class="color-input-wrapper">
                            <input 
                                ...
                                id="color" 
                                name="color" 
                                value="{{ old('color', $route->color ?? '#EBAF3E') }}"
                            >
                            <span class="color-value" id="colorValue">{{ old('color', $route->color ?? '#EBAF3E') }}</span>
                        </div>
                    </div>
            ...
            <div class="form-section map-section">
                ...
                <input type="hidden" id="path" name="path" value="{{ old('path', json_encode($route->path)) }}">
                <input type="hidden" id="waypoints" name="waypoints" value="{{ old('waypoints', json_encode($route->waypoints ?? $route->path)) }}">
